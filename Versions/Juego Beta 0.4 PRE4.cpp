...
void Aclaracion() {
	printf("\nBienvenido a mi juego en C versi%cn 0.4 PRE4\n\n\n",162);
...
void notas() {
	printf("Notas del Parche 0.4 PRE4:\n\n");
	printf("   %c Ahora los efectos de regeneraci%cn y veneno se acumulan\n",175,162);
	printf("   %c Powerup regeneraci%cn nerfeado\n",175,162);
	printf("   %c A%cadido contador en los efectos regeneraci%cn y veneno\n",175,164,162);
	printf("   %c Corregido bug: Los powerups no se te acumulaban entre niveles\n",175);
	printf("   %c Corregidos muchos bugs y a%cadidas muchas posibilidades en niveles altos",175,164);
	printf("\n\n");
	system("pause");
}
...
void dispAtributo(tEstado *estado) {
	if(estado->vidaregen>0) {
		printf("   REGENERACI%cN %d",224,estado->vidaregen);
	}
	if(estado->veneno>0) {
		printf("   VENENO %d",estado->veneno);
	}
}
...
tEstado *cambiaBloque(tEstado *estado, int bloque1, int bloque2) {
	...
	*nuevo=*estado;
	...
	switch(bloque2) {
		...
		case 4: extrai=3; extraj=0; break;
		case 5: if(bloque1==2) {
					extrai=3; extraj=0;
				}
				else {
...
tEstado *aplicaPowerup(tEstado *estado, char op) {
	...
	*nuevo=*estado;
	...
	if(op=='g'||op=='G') {
		nuevo->veneno=nuevo->veneno+2;
		for(j=nuevo->col+1;j<N;j++) {
			if(nuevo->matriz[nuevo->fila][j]!=4 && nuevo->matriz[nuevo->fila][j]!=3 && nuevo->matriz[nuevo->fila][j]!=8 && nuevo->matriz[nuevo->fila][j]!=10 && nuevo->matriz[nuevo->fila][j]!=12 && nuevo->matriz[nuevo->fila][j]!=13 && nuevo->matriz[nuevo->fila][j]!=14 && nuevo->matriz[nuevo->fila][j]!=15) {
				nuevo->matriz[nuevo->fila][j]=0;
			}
		}
		for(i=nuevo->fila+1;i<N;i++) {
			if(nuevo->matriz[i][nuevo->col]!=4 && nuevo->matriz[i][nuevo->col]!=3 && nuevo->matriz[i][nuevo->col]!=8 && nuevo->matriz[i][nuevo->col]!=10 && nuevo->matriz[i][nuevo->col]!=12 && nuevo->matriz[i][nuevo->col]!=13 && nuevo->matriz[nuevo->fila][j]!=14 && nuevo->matriz[nuevo->fila][j]!=15) {
...
tEstado *eliminaCopia(tEstado *estado) {
	tEstado *nuevo=(tEstado *)malloc(sizeof(tEstado));
	*nuevo=*estado;
	int i,j,repetido=0;
	for(i=0;i<N;i++) {
		for(j=0;j<N;j++) {
			if(nuevo->matriz[i][j]==5) {
				repetido++;
			}
		}
	}
...
int main() {
	...
	int nivel_6[N][N]={{0,0,0,0,0,12,0,0,4},{0,0,0,0,0,15,0,0,4},{0,0,12,0,0,15,14,0,4},{0,0,0,0,0,0,0,0,4},{0,0,0,0,0,0,0,0,4},{0,0,0,0,0,0,0,0,4},{0,0,0,0,0,0,0,0,4},{0,0,0,0,0,0,0,0,4},{4,4,4,4,4,4,4,4,4}};
	int nivel_prueba[N][N]={{0,0,0,1,1,1,2,2,2},{12,12,12,1,1,1,2,2,2},{0,12,0,1,1,1,2,2,2},{0,3,0,7,7,7,8,8,8},{3,3,3,7,7,7,8,8,8},{0,3,0,7,7,7,8,8,8},{6,6,6,9,9,9,0,0,0},{6,6,6,9,9,9,0,0,0},{6,6,6,9,9,9,0,0,0}};
	int k;
	char op;
	int nivel=1;
	...
	for(k=0;k!=-1;k++) {
		...
		if(estado->matriz[2][6]==5 && nivel==3) {
			...
			estado_Inicial->mana=estado->mana;
			estado=estado_Inicial;
			dispVida(estado);
			displlaves(estado);
			dispAtributo(estado);
				...
				if(nivel==2) {
					...
					if(nivel==3) {
						...
						if(nivel==5) {
							...
							estado_Inicial->objfila=4;
							estado_Inicial->objcol=6;
							estado_Inicial->mana=estado->mana+1;
							estado=estado_Inicial;
							dispVida(estado);
							displlaves(estado);
							dispAtributo(estado);
							...
							if(nivel==6) {
								...
								if(nivel==7) {
									printf("\n\n\n%c%c%c%c%c%c%c NIVEL 5 %c%c%c%c%c%c%c%c\n\n",175,175,175,175,175,175,175,174,174,174,174,174,174,174,174);
									estado_Inicial=crearEstado(nivel_6);
									estado_Inicial->vida=700;
									estado_Inicial->llaves=4;
									estado_Inicial->col=0;
			...
			if(esValido(estado,op) && (nivel==3 || nivel==5 || nivel==6)) {
				...
				if(esValido3(estado,op) && nivel==4) {
					...
					if(esValido(estado,op)) {
						...
						if(nivel==7 && nuevo->matriz[0][3]==5) {
							printf("\n\n");
							nuevo=cambiaBloque(nuevo,2,5);
							nuevo=eliminaCopia(nuevo);
							nuevo->fila=3;
							nuevo->col=3;
						}
						dispVida(nuevo);
