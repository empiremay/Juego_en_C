...
void Aclaracion() {
	printf("Bienvenido a mi juego en C version 0.1.1\n\n");
...
int esValido(tEstado *estado, int op) {
	int valido;
	switch(op) {
		case 3: valido=(estado->fila)>0; break;
		case 4: valido=estado->fila<(N-1); break;
		case 1: valido=estado->col<(N-1); break;
		case 2: valido=estado->col>0; break;
		default: valido=0;
	}
	return valido;
}
...
int main() {
	...
	tEstado *nuevo=(tEstado *)malloc(sizeof(tEstado));
	tEstado *estado=(tEstado *)malloc(sizeof(tEstado));
	int estado_inicial[N][N]={{5,1,0,2,0},{1,0,2,0,1},{1,1,2,2,0},{0,1,2,0,2},{2,0,1,1,3}};
	int k,op;
